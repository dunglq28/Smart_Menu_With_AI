...
import { DrawerComponent, Loading, ModalFormCreateMenu } from "@/components";
import { Images } from "@/assets";
import { CategoryService, MenuService, ProductService } from "@/services";
import { CategoryData, ProductData } from "@/payloads";
...
function CreateMenu() {
  ...
  const handleChangeProductByCate = (cateId: number) => {
    const loadData = async () => {
      try {
        const result = await ProductService.getProductsByCategory(brandId, cateId);
        if (result) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const result = await CategoryService.getCategoriesByBrandId(brandId);
  ...
  const handleCreateMenu = async (menuForm: FormData) => {
    try {
      setIsLoading(true);
      const menuResult = await MenuService.createMenu(menuForm);
      ...
      if (menuResult.statusCode === 200) {
        ...
        const listPositionResult = await MenuService.createListPosition(allListProducts, brandId);
        ...
        if (listPositionResult.statusCode === 200) {
          ...
          const menuListResult = await MenuService.createMenuList(
          ...
          if (menuListResult.statusCode === 200) {
            ...
            const productListResult = await MenuService.createProductList(
              brandId,
              listProductDetails,
            );
  ...
  useEffect(() => {
    if (menuId !== "") {
      ...
      try {
        const loadData = async () => {
          var result = await MenuService.getMenu(menuId);
  ...
  const handleUpdateMenu = async (menuForm: FormData) => {
    try {
      setIsLoading(true);
      const menuResult = await MenuService.updateMenu(menuForm);
      ...
      if (menuResult.statusCode === 200) {
        ...
        const listPositionResult = await MenuService.updateListPosition(allListProducts, brandId);
        ...
        if (listPositionResult.statusCode === 200) {
          ...
          const productListResult = await MenuService.updateProductList(
            brandId,
            listProductDetails,
          );
    ...
    <Flex className={style.Container}>
      ...
      <Flex className={style.CardContainer}>
        <Flex className={style.Card} bg="#E3F2F1" onClick={onOpenCreateMenu}>
          <Image className={style.MenuImg} src={Images.fakeMenu} />
        </Flex>
      </Flex>
      <ModalFormCreateMenu
