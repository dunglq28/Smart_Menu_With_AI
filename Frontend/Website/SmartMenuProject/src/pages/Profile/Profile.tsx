...
import { Billing, ProfileInfo, SecuritySettings } from "@/components";
import { BrandService, SubscriptionsService, UserService } from "@/services";
import { BrandData, SubscriptionData, UserData } from "@/payloads";
...
function Profile() {
  ...
  const getBrandById = async (): Promise<BrandData | null> => {
    try {
      ...
      const response = await BrandService.getBrand(Number(brandId));
      if (!response.isSuccess) {
