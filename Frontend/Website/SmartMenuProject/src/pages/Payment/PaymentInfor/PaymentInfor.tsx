...
import { PackageDetails, RegistrationForm } from "@/components";
import { brandUpdate, PlanData, userUpdate } from "@/payloads";
...
const PaymentInfoPage = () => {
  ...
  async function handleSendVerificationCode() {
    ...
    const result = await EmailService.sendVerificationCode(email.value, code);
    if (result.isSuccess) {
  ...
  useEffect(() => {
    ...
    if (countdown > 0) {
      ...
      setExpiredCodes((prev) => [...prev, verificationCode]);
      setVerificationCode("");
      setIsCodeSent(false);
    }
  ...
  async function addNewBrand(): Promise<BrandResult> {
    try {
      ...
      if (brandForm.image.value && brandForm.brandName.value) {
        ...
        const response = await BrandService.getBrandByBrandName(brandForm.brandName.value);
        if (response.data == null) {
          const userResult = await UserService.createUser(userForm, 2);
          if (userResult.statusCode === 200) {
            brandFormRequest.append("UserId", userResult.data.toString());
            const brandResult = await BrandService.createBrand(brandFormRequest);
  ...
  async function updateOldBrand(userId: number): Promise<BrandResult> {
    try {
      ...
      var brand = await BrandService.getBrandByUserId(userId);
      ...
      var brandResult = await BrandService.updateBrand(brandUpdate.id, brandUpdate);
      ...
      var userResult = await UserService.updateUser(userId, userUpdate, null);
  ...
  async function handleCreatePaymentLink() {
    ...
    if (!hasError) {
      try {
        setIsLoading(true);
        const payment = await PaymentService.checkExistEmail(email.value);
