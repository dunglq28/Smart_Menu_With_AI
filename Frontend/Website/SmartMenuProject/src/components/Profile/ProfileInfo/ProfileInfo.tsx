...
import { themeColors, UserRole } from "@/constants";
import {
  isImageFile,
  validateBrandForm,
  validateUserForm,
...
import { BrandData, brandUpdate, UserData, userUpdate } from "@/payloads";
  ...
  const handleUpdate = async () => {
    ...
    if (!hasError) {
      ...
      if (isUserChanged) {
        ...
        userResult = await UserService.updateUser(userData?.userId!, userUpdate, null);
      }
      ...
      if (isBrandChanged) {
        ...
        brandResult = await BrandService.updateBrand(brandUpdate.id, brandUpdate);
      }
